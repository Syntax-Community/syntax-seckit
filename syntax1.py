...
def crawl_for_link(target_url, max_depth=1):
...
def main():
    ...
    parser_crawl = subparsers.add_parser('crawl', help='Crawler untuk mengumpulkan path url dari target.')
    ...
    parser_link = subparsers.add_parser('link', help='Mengumpulkan subdomain dari URL target.')
    parser_link.add_argument('url', help='URL target (misal: https://domain.com).')
    parser_link.add_argument('--depth', type=int, default=1, help='Kedalaman crawling maksimum (Default: 1).')
    parser_link.set_defaults(func=lambda args: crawl_for_link(args.url, args.depth))
